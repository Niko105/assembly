...
section .text ;code
_start:
    mov rbx, [rsp] ;save argc
    mov rsi, [rsp+16] ;first argument is the program name, so go to the second

    ;rsi is set up already
    call stringLength ;get the length of the string in rsi to rax
    mov rdx, rax ;save it to rdx

    mov rax, syscallWrite ;write()
    mov rdi, 1 ;stdout
    mov rsi, rsi ;write the second argument, still setup
    mov rdx, rdx ;string ;lenght
    syscall
